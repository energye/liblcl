...
type
  ...
  public
    ...
    procedure Internal_OnBeforeNavigation(const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest; navigationType: TCefNavigationType;
    ...
    procedure Internal_OnUncaughtException(const browser: ICefBrowser; const frame: ICefFrame; const context: ICefv8Context; const Exception: ICefV8Exception;
...
begin
  Result := IntToStr(CEF_SUPPORTED_VERSION_MAJOR) + '.' + IntToStr(CEF_SUPPORTED_VERSION_MINOR) + '.' + IntToStr(CEF_SUPPORTED_VERSION_RELEASE) +
...
begin
  FCookies := CustomAbsolutePath(aValue);
end;
...
function TCefApplication.GetLocalesDirPath: ustring;
begin
...
function TCefApplication.GetResourcesDirPath: ustring;
begin
...
begin
  ...
  begin
    ...
    TempMissingRsc := not (CheckResources(ResourcesDirPath, FMissingLibFiles, FCheckDevToolsResources));
    ...
    else if CheckDLLVersion(LibCefPath, CEF_SUPPORTED_VERSION_MAJOR, CEF_SUPPORTED_VERSION_MINOR, CEF_SUPPORTED_VERSION_RELEASE, CEF_SUPPORTED_VERSION_BUILD) then
    begin
        ...
        case TempMachine of
          CEF_IMAGE_FILE_MACHINE_I386:
            ...
            begin
              ...
              TempString :=
                'Wrong CEF3 binaries !' + CRLF + CRLF + 'Use the 32 bit CEF3 binaries with 32 bits applications only.';
          ...
          CEF_IMAGE_FILE_MACHINE_AMD64:
            ...
            begin
              FStatus := asErrorDLLVersion;
              TempString :=
                'Wrong CEF3 binaries !' + CRLF + CRLF + 'Use the 64 bit CEF3 binaries with 64 bits applications only.';
          ...
          begin
            FStatus := asErrorDLLVersion;
            TempString :=
              'Unknown CEF3 binaries !' + CRLF + CRLF + 'Use only the CEF3 binaries specified in the CEF4Delphi Readme.md file at ' + CEF4DELPHI_URL;
    ...
    begin
      FStatus := asErrorDLLVersion;
      TempString := 'Unsupported CEF version !' + CRLF + CRLF + 'Use only the CEF3 binaries specified in the CEF4Delphi Readme.md file at ' + CEF4DELPHI_URL;

      if GetDLLVersion(LibCefPath, TempVersionInfo) then
        TempString := TempString + CRLF + CRLF + 'Expected ' + LIBCEF_DLL + ' version : ' + LibCefVersion + CRLF + 'Found ' + LIBCEF_DLL +
          ' version : ' + FileVersionInfoToString(TempVersionInfo);
...
begin
  ...
  try
    if not (FSingleProcess) and (FStatus = asLoading) and (ProcessType <> ptBrowser) and LoadCEFlibrary then
    begin
      TempApp := TCustomCefApp.Create(self);
...
begin
  ...
  try
    ...
    begin
      ...
      if (TempCookieManager <> nil) and TempCookieManager.SetStoragePath(FCookies, FPersistSessionCookies, nil) then
        Result := True
      else
        OutputDebugMessage(
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        repeat
          if (TempSearchRec.Attr <> faDirectory) and GetStringFileInfo(TempPath + TempSearchRec.Name, 'ProductName', TempProductName) and
...
procedure TCefApplication.Internal_OnBeforeNavigation(const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest;
...
procedure TCefApplication.Internal_OnUncaughtException(const browser: ICefBrowser; const frame: ICefFrame; const context: ICefv8Context;
...
procedure TCefApplication.Internal_OnBeforeCommandLineProcessing(const processType: ustring; const commandLine: ICefCommandLine);
var
  i: integer;
  TempVersionInfo: TFileVersionInfo;
  TempFileName: string;
begin
  ...
  begin
    if FindFlashDLL(TempFileName) and GetDLLVersion(TempFileName, TempVersionInfo) then
    begin
...
begin
  Result := FSingleProcess or ((FProcessType in [ptBrowser, ptRenderer]) and (FMustCreateResourceBundleHandler or assigned(FOnGetLocalizedString) or
    assigned(FOnGetDataResource) or assigned(FOnGetDataResourceForScale)));
end;

function TCefApplication.GetMustCreateBrowserProcessHandler: boolean;
begin
  Result := FSingleProcess or ((FProcessType = ptBrowser) and (FMustCreateBrowserProcessHandler or assigned(FOnContextInitialized) or
    assigned(FOnBeforeChildProcessLaunch) or assigned(FOnRenderProcessThreadCreated)));
end;

function TCefApplication.GetMustCreateRenderProcessHandler: boolean;
begin
  Result := FSingleProcess or ((FProcessType = ptRenderer) and (FMustCreateRenderProcessHandler or MustCreateLoadHandler or assigned(FOnRenderThreadCreated) or
    assigned(FOnWebKitInitialized) or assigned(FOnBrowserCreated) or assigned(FOnBrowserDestroyed) or assigned(FOnContextCreated) or
    assigned(FOnContextReleased) or assigned(FOnUncaughtException) or assigned(FOnFocusedNodeChanged) or assigned(FOnProcessMessageReceived)));
end;

function TCefApplication.GetMustCreateLoadHandler: boolean;
begin
  Result := FSingleProcess or ((FProcessType = ptRenderer) and (FMustCreateLoadHandler or assigned(FOnLoadingStateChange) or assigned(FOnLoadStart) or
...
begin
  ...
  begin
    FStatus := asErrorLoadingLibrary;
    TempString := 'Error loading libcef.dll' + CRLF + CRLF + 'Error code : 0x' + inttohex(GetLastError, 8);
  ...
  if Load_cef_app_capi_h and Load_cef_browser_capi_h and Load_cef_command_line_capi_h and Load_cef_cookie_capi_h and Load_cef_drag_data_capi_h and
    Load_cef_geolocation_capi_h and Load_cef_origin_whitelist_capi_h and Load_cef_parser_capi_h and Load_cef_path_util_capi_h and
    Load_cef_print_settings_capi_h and Load_cef_process_message_capi_h and Load_cef_process_util_capi_h and Load_cef_request_capi_h and
    Load_cef_request_context_capi_h and Load_cef_resource_bundle_capi_h and Load_cef_response_capi_h and Load_cef_scheme_capi_h and
    Load_cef_stream_capi_h and Load_cef_task_capi_h and Load_cef_trace_capi_h and Load_cef_urlrequest_capi_h and Load_cef_v8_capi_h and
    Load_cef_values_capi_h and Load_cef_web_plugin_capi_h and Load_cef_xml_reader_capi_h and Load_cef_zip_reader_capi_h and Load_cef_logging_internal_h and
    Load_cef_string_list_h and Load_cef_string_map_h and Load_cef_string_multimap_h and Load_cef_string_types_h and Load_cef_thread_internal_h and Load_cef_trace_event_internal_h then
  begin
    FStatus := asLoaded;
    FLibLoaded := True;
    Result := True;
  ...
  begin
    FStatus := asErrorDLLVersion;
    TempString := 'Unsupported CEF version !' + CRLF + CRLF + 'Use only the CEF3 binaries specified in the CEF4Delphi Readme.md file at ' + CRLF + CEF4DELPHI_URL;
...
Pointer(
  ...
  Result := assigned(cef_initialize) and assigned(cef_shutdown) and assigned(cef_execute_process) and assigned(cef_do_message_loop_work) and
    assigned(cef_run_message_loop) and assigned(cef_quit_message_loop) and assigned(cef_set_osmodal_loop) and assigned(cef_enable_highdpi_support);
end;
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_cookie_manager_create_manager');
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_get_geolocation');
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_parse_csscolor');
  {$IFDEF FPC}
Pointer(
  {$ENDIF}
  cef_parse_json
...
Pointer(
  ...
  Result := assigned(cef_parse_url) and assigned(cef_create_url) and assigned(cef_format_url_for_security_display) and assigned(cef_get_mime_type) and
    assigned(cef_get_extensions_for_mime_type) and assigned(cef_base64encode) and assigned(cef_base64decode) and assigned(cef_uriencode) and
    assigned(cef_uridecode) and assigned(cef_parse_csscolor) and assigned(cef_parse_json) and assigned(cef_parse_jsonand_return_error) and assigned(cef_write_json);
end;
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'create_context_shared');

  Result := assigned(cef_request_context_get_global_context) and assigned(cef_request_context_create_context) and assigned(create_context_shared);
end;
...
Pointer(
  ...
  Result := assigned(cef_stream_reader_create_for_file) and assigned(cef_stream_reader_create_for_data) and assigned(cef_stream_reader_create_for_handler) and
    assigned(cef_stream_writer_create_for_file) and assigned(cef_stream_writer_create_for_handler);
end;
...
Pointer(
  ...
  Result := assigned(cef_task_runner_get_for_current_thread) and assigned(cef_task_runner_get_for_thread) and assigned(cef_currently_on) and
    assigned(cef_post_task) and assigned(cef_post_delayed_task);
end;
...
Pointer(
  ...
  Result := assigned(cef_v8context_get_current_context) and assigned(cef_v8context_get_entered_context) and assigned(cef_v8context_in_context) and
    assigned(cef_v8value_create_undefined) and assigned(cef_v8value_create_null) and assigned(cef_v8value_create_bool) and assigned(cef_v8value_create_int) and
    assigned(cef_v8value_create_uint) and assigned(cef_v8value_create_double) and assigned(cef_v8value_create_date) and assigned(cef_v8value_create_string) and
    assigned(cef_v8value_create_object) and assigned(cef_v8value_create_array) and assigned(cef_v8value_create_function) and assigned(cef_v8stack_trace_get_current) and
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_add_web_plugin_path');
  {$IFDEF FPC}
Pointer(
  {$ENDIF}
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_add_web_plugin_directory');
  {$IFDEF FPC}
Pointer(
  {$ENDIF}
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_remove_web_plugin_path');
  {$IFDEF FPC}
Pointer(
  {$ENDIF}
...
Pointer(
  ...
  {$ENDIF}
  := GetProcAddress(FLibHandle, 'cef_force_web_plugin_shutdown');
  {$IFDEF FPC}
Pointer(
  {$ENDIF}
...
Pointer(
  ...
  Result := assigned(cef_visit_web_plugin_info) and assigned(cef_refresh_web_plugins) and assigned(cef_add_web_plugin_path) and
    assigned(cef_add_web_plugin_directory) and assigned(cef_remove_web_plugin_path) and assigned(cef_unregister_internal_web_plugin) and
    assigned(cef_force_web_plugin_shutdown) and assigned(cef_register_web_plugin_crash) and assigned(cef_is_web_plugin_unstable);
end;
...
Pointer(
  ...
  Result := assigned(cef_string_list_alloc) and assigned(cef_string_list_size) and assigned(cef_string_list_value) and assigned(cef_string_list_append) and
    assigned(cef_string_list_clear) and assigned(cef_string_list_free) and assigned(cef_string_list_copy);
end;
...
Pointer(
  ...
  Result := assigned(cef_string_map_alloc) and assigned(cef_string_map_size) and assigned(cef_string_map_find) and assigned(cef_string_map_key) and
    assigned(cef_string_map_value) and assigned(cef_string_map_append) and assigned(cef_string_map_clear) and assigned(cef_string_map_free);
end;
...
Pointer(
  ...
  Result := assigned(cef_string_multimap_alloc) and assigned(cef_string_multimap_size) and assigned(cef_string_multimap_find_count) and
    assigned(cef_string_multimap_enumerate) and assigned(cef_string_multimap_key) and assigned(cef_string_multimap_value) and
    assigned(cef_string_multimap_append) and assigned(cef_string_multimap_clear) and assigned(cef_string_multimap_free);
end;
...
Pointer(
  ...
  Result := assigned(cef_string_wide_set) and assigned(cef_string_utf8_set) and assigned(cef_string_utf16_set) and assigned(cef_string_wide_clear) and
    assigned(cef_string_utf8_clear) and assigned(cef_string_utf16_clear) and assigned(cef_string_wide_cmp) and assigned(cef_string_utf8_cmp) and
    assigned(cef_string_utf16_cmp) and assigned(cef_string_wide_to_utf8) and assigned(cef_string_utf8_to_wide) and assigned(cef_string_wide_to_utf16) and
    assigned(cef_string_utf16_to_wide) and assigned(cef_string_utf8_to_utf16) and assigned(cef_string_utf16_to_utf8) and assigned(cef_string_ascii_to_wide) and
    assigned(cef_string_ascii_to_utf16) and assigned(cef_string_userfree_wide_alloc) and assigned(cef_string_userfree_utf8_alloc) and
    assigned(cef_string_userfree_utf16_alloc) and assigned(cef_string_userfree_wide_free) and assigned(cef_string_userfree_utf8_free) and assigned(cef_string_userfree_utf16_free);
end;
...
Pointer(
  ...
  Result := assigned(cef_trace_event_instant) and assigned(cef_trace_event_begin) and assigned(cef_trace_event_end) and assigned(cef_trace_counter) and
    assigned(cef_trace_counter_id) and assigned(cef_trace_event_async_begin) and assigned(cef_trace_event_async_step_into) and
    assigned(cef_trace_event_async_step_past) and assigned(cef_trace_event_async_end);
end;
