...
type
  TCefClientRef = class(TCefBaseRef, ICefClient)
    protected
      ...
      procedure GetGeolocationHandler(var aHandler : ICefGeolocationHandler); virtual;
  ...
  TCustomClientHandler = class(TCefClientOwn)
    protected
      ...
      procedure GetGeolocationHandler(var aHandler : ICefGeolocationHandler); override;
...
uses
  ...
  uCEFDisplayHandler, uCEFDownloadHandler, uCEFJsDialogHandler, uCEFGeolocationHandler,
...
procedure TCefClientRef.GetGeolocationHandler(var aHandler : ICefGeolocationHandler);
begin
  aHandler := nil;
end;
...
var
  TempObject  : TObject;
  TempHandler : ICefGeolocationHandler;
begin
  Result      := nil;
  TempObject  := CefGetObject(self);
  ...
  if (TempObject <> nil) and (TempObject is TCefClientOwn) then
    try
      TCefClientOwn(TempObject).GetGeolocationHandler(TempHandler);
      if (TempHandler <> nil) then Result := TempHandler.Wrap;
    finally
      TempHandler := nil;
    end;
...
begin
  ...
  with PCefClient(FData)^ do
    begin
      ...
      get_geolocation_handler     := {$IFDEF FPC}@{$ENDIF}cef_client_own_get_geolocation_handler;
...
procedure TCefClientOwn.GetGeolocationHandler(var aHandler : ICefGeolocationHandler);
begin
  aHandler := nil;
end;
...
procedure TCustomClientHandler.GetGeolocationHandler(var aHandler : ICefGeolocationHandler);
begin
