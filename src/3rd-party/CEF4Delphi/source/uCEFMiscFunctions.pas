...
uses
  ...
  {$ELSE}
  Windows, {$IFDEF DELPHI14_UP}IOUtils,{$ENDIF} Classes, SysUtils, Controls, ActiveX, Math,
  {$ENDIF}
...
function CefUserFreeString(const str: ustring): PCefStringUserFree;
begin
  Result        := {$IFDEF FPC}@{$ENDIF}cef_string_userfree_utf16_alloc;
  Result^.length := Length(str);
...
begin
  ...
  {$ENDIF}
  Result := cef_execute_process(@TempArgs, CefGetData(app), aWindowsSandboxInfo);
end;
...
begin
  ...
  try
    try
      ...
      TempList.Add(TempDir + 'wow_helper.exe');
