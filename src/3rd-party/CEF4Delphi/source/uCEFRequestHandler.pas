...
type
  TCefRequestHandlerOwn = class(TCefBaseOwn, ICefRequestHandler)
    protected
      ...
      procedure OnResourceRedirect(const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest; var newUrl: ustring); virtual;
  ...
  TCustomRequestHandler = class(TCefRequestHandlerOwn)
    protected
      ...
      procedure OnResourceRedirect(const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest; var newUrl: ustring); override;
...
uses
  ...
  uCEFResponse, uCEFAuthCallback, uCEFSslInfo, uCEFApplication;
...
procedure TCustomRequestHandler.OnResourceRedirect(const browser  : ICefBrowser;
                                                   ...
                                                   var   newUrl   : ustring);
begin
  if (FEvents <> nil) then IChromiumEvents(FEvents).doOnResourceRedirect(browser, frame, request, newUrl);
end;
