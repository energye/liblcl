...
const
  ...
  MINIBROWSER_HOMEPAGE = 'https://www.baidu.com';
...
type
  TMiniBrowserFrm = class(TForm)
    ...
    procedure Chromium1ResourceResponse(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest;
...
procedure TMiniBrowserFrm.Chromium1BeforeContextMenu(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const params: ICefContextMenuParams; const model: ICefMenuModel);
begin
...
procedure TMiniBrowserFrm.Chromium1BeforeDownload(Sender: TObject; const browser: ICefBrowser; const downloadItem: ICefDownloadItem;
...
begin
  ...
  if Chromium1.IsSameBrowser(browser) and (frame <> nil) and frame.IsMain then
...
procedure TMiniBrowserFrm.Chromium1ContextMenuCommand(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const params: ICefContextMenuParams;
  commandId: integer; eventFlags: cardinal; out Result: boolean);
var
  TempParam: WParam;
begin
...
begin
  TempMessage := 'Total memory used by this application : ' + IntToStr(GlobalCEFApp.UsedMemory div BYTES_PER_MEGABYTE) + ' Mb' + CRLF +
    'Total system memory : ' + IntToStr(GlobalCEFApp.TotalSystemMemory div BYTES_PER_MEGABYTE) + ' Mb' + CRLF + 'Available physical memory : ' +
...
procedure TMiniBrowserFrm.Chromium1ResourceResponse(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest;
  const response: ICefResponse; out Result: boolean);
begin
  Result := False;
...
procedure TMiniBrowserFrm.OpenfilewithaDATAURL1Click(Sender: TObject);
var
  TempDATA: string;
